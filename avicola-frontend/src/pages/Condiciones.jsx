...
export default function Condiciones() {
  ...
  async function handleUpdate() {
    if (!registroEdit.temperatura || !registroEdit.humedad) return
    
    setGuardando(true)
    try {
      const actualizado = await api.put(
        `/condiciones/${registroEdit.id}?idGalpon=${registroEdit.id_galpon}`,
        { 
          fecha:       registroEdit.fecha,
          temperatura: Number(registroEdit.temperatura),
          humedad:     Number(registroEdit.humedad),
          ventilacion: registroEdit.ventilacion,
          iluminacion: registroEdit.iluminacion,
        }
      )
  ...
  async function handleEliminar() {
    setGuardando(true)
    try {
      await api.delete(`/condiciones/${registroElim}`)
      dispatch({ type: 'DELETE_CONDICION', payload: { id: registroElim } })
  ...
  return (
    <Layout titulo="🌡️ Clima y Ambiente">
      ...
      {registroEdit && (
        <Modal 
          titulo={`Editar Clima - ${galponNombre(registroEdit.id_galpon)}`}
          ...
          <div className="space-y-4">
            <Campo label="Temperatura (°C)">
              <input 
                ...
                step="0.1"
                className="input-field text-xl font-bold"
                value={registroEdit.temperatura}
                onChange={e => setRegistroEdit({...registroEdit, temperatura: e.target.value})}
              />
              <TempIndicador valor={registroEdit.temperatura} />
            </Campo>
            <Campo label="Humedad (%)">
              <input 
                type="number" 
                step="0.1"
                className="input-field text-xl font-bold"
                value={registroEdit.humedad}
                onChange={e => setRegistroEdit({...registroEdit, humedad: e.target.value})}
              />
              <HumedadIndicador valor={registroEdit.humedad} />
            </Campo>
            <div className="grid grid-cols-2 gap-4">
              <Campo label="Ventilación">
                <select className="input-field" value={registroEdit.ventilacion}
                  onChange={e => setRegistroEdit({ ...registroEdit, ventilacion: e.target.value })}>
                  {ventilaciones.map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </Campo>
              <Campo label="Iluminación">
                <select className="input-field" value={registroEdit.iluminacion}
                  onChange={e => setRegistroEdit({ ...registroEdit, iluminacion: e.target.value })}>
                  {iluminaciones.map(il => <option key={il} value={il}>{il}</option>)}
                </select>
              </Campo>
            </div>
          </div>
      ...
      {registroElim && (
        <ModalConfirm 
          ...
          mensaje="Se borrará el registro de condiciones ambientales. Esta acción no se puede deshacer."
