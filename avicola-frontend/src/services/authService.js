...
export async function registerWithEmail({ nombre, email, password, rol = 'usuario' }) {
  ...
  // Mapear el rol seleccionado al rol del sistema
  const rolSistema = rol === 'admin' ? 'admin' : 'operario'
  const profile = { nombre: nombre.trim(), rol: rolSistema, email: normalizedEmail }
