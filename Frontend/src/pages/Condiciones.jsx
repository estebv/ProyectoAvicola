...
import { Campo, SelectorGalpon, Toast, FilaVacia, Badge } from '../components/ui'
import { Plus, Save, Thermometer } from 'lucide-react'
...
function TempIndicador({ valor }) {
  ...
  if (!v) return null
  const ok  = v >= 18 && v <= 28
  const warn = (v > 28 && v <= 32) || (v < 18 && v >= 15)
  const bad  = v > 32 || v < 15
  return (
...
export default function Condiciones() {
  ...
  function guardar(e) {
    ...
    // Guarda condiciones para monitoreo y reportes ambientales.
    dispatch({ type: 'ADD_CONDICION', payload: { ...form, id_galpon: Number(form.id_galpon), temperatura: Number(form.temperatura), humedad: Number(form.humedad) } })
    setToast({ tipo: 'exito', mensaje: '✅ Condiciones ambientales guardadas' })
    setForm(formVacio)
  ...
  function editarRegistro(registro) {
    ...
    dispatch({ type: 'UPDATE_CONDICION', payload: { id: registro.id, temperatura, humedad } })
  }

  function eliminarRegistro(id) {
    if (!admin) return
    if (!confirm('¿Eliminar este registro ambiental?')) return
          ...
          <form onSubmit={guardar} className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Campo label="💧 Humedad (%)" requerido error={errores.humedad}
                ...
                <input type="number" step="0.1" min="0" max="100" placeholder="Ej: 65" className="input-field text-xl font-bold"
                  value={form.humedad}
                ...
                <div className="grid grid-cols-5 gap-1.5">
                  {ventilaciones.map(v => (
                    <button key={v} type="button"
                      onClick={() => setForm({ ...form, ventilacion: v })}
              ...
              <Campo label="💡 Horas de luz" ayuda="¿Cuántas horas de luz recibieron?">
                <div className="grid grid-cols-3 gap-2">
                  {iluminaciones.map(il => (
                    <button key={il} type="button"
                      onClick={() => setForm({ ...form, iluminacion: il })}
                      ...
                      {admin && (
                        <td className="px-4 py-3 text-right">
                          <div className="flex gap-2 justify-end">
                            <button type="button" className="btn-secondary !px-3 !py-1.5" onClick={() => editarRegistro(c)}>Editar</button>
                            <button type="button" className="btn-danger !px-3 !py-1.5" onClick={() => eliminarRegistro(c.id)}>Eliminar</button>
                          </div>
                        </td>
                      )}
                    </tr>
