...
export default function Huevos() {
  ...
  function guardar(e) {
    ...
    // Guarda producción en el contexto central para que impacte dashboards/reportes.
    dispatch({ type: 'ADD_HUEVO', payload: { ...form, id_galpon: Number(form.id_galpon), total_huevo: Number(form.total_huevo), peso_huevo: Number(form.peso_huevo) } })
    setToast({ tipo: 'exito', mensaje: '✅ Producción guardada correctamente' })
    setForm(formVacio)
  ...
  function editarRegistro(registro) {
    ...
    dispatch({ type: 'UPDATE_HUEVO', payload: { id: registro.id, total_huevo: total, peso_huevo: peso } })
  }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <Campo label="¿Cuántos huevos?" requerido error={errores.total_huevo}
                ...
                <input type="number" min="0" placeholder="Ej: 3200" className="input-field text-xl font-bold"
                  value={form.total_huevo}
                  onChange={e => setForm({ ...form, total_huevo: e.target.value })} />
              </Campo>
              ...
              <Campo label="Peso del huevo (gramos)" requerido error={errores.peso_huevo}
                ...
                <input type="number" min="0" step="0.1" placeholder="Ej: 58.5" className="input-field"
                  value={form.peso_huevo}
                  onChange={e => setForm({ ...form, peso_huevo: e.target.value })} />
              </Campo>
            </div>

            <Campo label="¿Cómo es la calidad del huevo?" ayuda="Selecciona cómo se ven los huevos hoy">
              <CalidadSelector
                value={form.calidad_huevo}
                    ...
                    {admin && (
                      <td className="px-4 py-3 text-right">
                        <div className="flex gap-2 justify-end">
                          <button type="button" className="btn-secondary !px-3 !py-1.5" onClick={() => editarRegistro(h)}>Editar</button>
                          <button type="button" className="btn-danger !px-3 !py-1.5" onClick={() => eliminarRegistro(h.id)}>Eliminar</button>
                        </div>
                      </td>
                    )}
                  </tr>
