...
export default function Alimentos() {
  ...
  function guardar(e) {
    ...
    // Registra consumo para análisis posterior por galpón.
    dispatch({ type: 'ADD_ALIMENTO', payload: { ...form, id_galpon: Number(form.id_galpon), cantidad_kg: Number(form.cantidad_kg) } })
    setToast({ tipo: 'exito', mensaje: '✅ Consumo de alimento guardado' })
    setForm(formVacio)
  ...
  function editarRegistro(registro) {
    ...
    dispatch({ type: 'UPDATE_ALIMENTO', payload: { id: registro.id, cantidad_kg: cantidad } })
  }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Campo label="¿Cuántos kilogramos?" requerido error={errores.cantidad_kg}
                ...
                <div className="relative">
                  <input type="number" min="0" step="0.1" placeholder="Ej: 480" className="input-field pr-12 text-xl font-bold"
                    ...
                    {admin && (
                      <td className="px-4 py-3 text-right">
                        <div className="flex gap-2 justify-end">
                          <button type="button" className="btn-secondary !px-3 !py-1.5" onClick={() => editarRegistro(a)}>Editar</button>
                          <button type="button" className="btn-danger !px-3 !py-1.5" onClick={() => eliminarRegistro(a.id)}>Eliminar</button>
                        </div>
                      </td>
                    )}
                  </tr>
